...
CREATE TABLE IF NOT EXISTS user_addresses (
  ...
  label TEXT NOT NULL, -- "بيتي", "الشغل", "أهلي", etc.
...
CREATE TABLE IF NOT EXISTS user_payment_methods (
  id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
  user_id UUID REFERENCES users(id) ON DELETE CASCADE,
  type TEXT NOT NULL, -- 'cash', 'credit_card', 'vodafone_cash', 'instapay', 'fawry'
  is_default BOOLEAN DEFAULT FALSE,
  -- For card payments (encrypted/not stored - placeholder for future integration)
  card_last_four TEXT,
...
CREATE TABLE IF NOT EXISTS user_preferences (
  id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
  user_id UUID REFERENCES users(id) ON DELETE CASCADE,
  preferred_service_time TEXT, -- 'morning', 'afternoon', 'evening'
  preferred_worker_id UUID REFERENCES workers(id),
  car_care_notes TEXT, -- "عربيتي حساسة", "مش عايز مياه كتير", etc.
  allergies TEXT, -- For interior cleaning materials
  created_at TIMESTAMP DEFAULT NOW(),
  updated_at TIMESTAMP DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS car_photos (
  ...
  photo_type TEXT DEFAULT 'general', -- 'before', 'after', 'general', 'damage'
...
CREATE TABLE IF NOT EXISTS user_activity_log (
  id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
  user_id UUID REFERENCES users(id) ON DELETE CASCADE,
  activity_type TEXT NOT NULL, -- 'login', 'order_placed', 'profile_updated', 'car_added'
  activity_data JSONB,
  ip_address TEXT,
  user_agent TEXT,
  created_at TIMESTAMP DEFAULT NOW()
